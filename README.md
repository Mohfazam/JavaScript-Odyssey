...
9. Check if a number is prime. :white_check_mark:  
...
#### **Intermediate (Promise: 6-10)**
Promise:6. Write a function that chains multiple promises together and logs the result after all are resolved.  
Promise:7. Create a function that fetches user data from two different APIs in sequence using promises.  
Promise:8. Implement a promise-based function that resolves after fetching data from multiple APIs using `Promise.all()`.  
Promise:9. Write a function that retries fetching data from an API up to three times if it fails.  
Promise:10. Implement a promise-based timeout function that rejects if the operation takes longer than 5 seconds.

#### **Advanced (Promise: 11-15)**
Promise:11. Write an async function that waits for multiple promises to resolve using `Promise.all()` and logs the results.  
Promise:12. Implement a function that uses `async/await` to fetch data from an API and handles errors with `try/catch`.  
Promise:13. Create a function that fetches data from an API and retries a failed request three times using `async/await`.  
Promise:14. Write an async function that processes an array of items in parallel and logs the results.  
Promise:15. Implement a function that fetches data from multiple APIs and cancels the request if it takes too long.

#### **Expert (Promise: 16-20)**
Promise:16. Write a function that implements an exponential backoff strategy when retrying a failed promise.  
Promise:17. Create a function that throttles API requests using promises to ensure only a set number of requests happen concurrently.  
Promise:18. Build a promise-based task queue that processes tasks sequentially, even if the tasks themselves are asynchronous.  
Promise:19. Write a function that uses `Promise.race()` to handle multiple API requests and return the result of the fastest one.  
Promise:20. Implement a promise-based batching mechanism that sends multiple requests in batches of 5 and waits for all of them to complete before proceeding.

#### **Bonus (Promise: 21-25)**
Promise:21. Implement a custom `Promise.all()` polyfill.  
Promise:22. Write a function that wraps a callback-based function into a promise and provides both `resolve` and `reject` logic.  
